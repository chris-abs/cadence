...
>(({ className, border, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn(border ? '[&[data-state=open]]:border-b' : '', className)}
    {...props}
  />
))
...
>(({ className, parent, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        'flex flex-1 items-center justify-between text- font-medium transition-all text-left [&[data-state=open]>svg]:rotate-180',
        parent
